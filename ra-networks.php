...
if ( $_POST ) {
	switch($_POST['action']) {
		...
		case 'deletesite':
			...
			do_action( 'netplus_deletesite', $network_ids );
			break;
	}
}	?>
<div class="wrap">
	...
	?><h2>Add Network</h2>
		<table class="form-table">  
			<tr> 
				...
				<td>
					<input type='text' name='basedomain' value='<?php echo isset( $domain ) ? $domain : ''; ?>' />
			...
			<tr> 
				...
				<td>
					<input name='weblog_title' type='text' size='45' value='<?php echo isset( $weblog_title ) ? $weblog_title : ''; ?>' />
...
if ( ! defined( 'BP_ENABLE_MULTIBLOG' ) && ! defined( 'BP_ROOT_BLOG' ) && isset( $current_site->blog_id ) )
...
function ra_populate_network( $network_id = 1, $domain = '', $email = '', $site_name = '', $path = '/', $subdomain_install = false ) {
	...
	// ensure path has a valid path in the event that a DM plugin is not assigning $current_site->path a value
	if ( empty( $path ) )
		$path = '/';
...
--The Team @ SITE_NAME' );
	...
	foreach( $sitemeta as $meta_key => $meta_value ) {
		...
		$wpdb->insert( $wpdb->sitemeta, array( 'site_id' => $network_id, 'meta_key' => $meta_key, 'meta_value' => $meta_value ) );
	}
	...
	if( is_multisite() ) {
		...
		if ( ! $base_path = get_option( 'upload_path' ) )
			$base_path = substr( WP_CONTENT_DIR, strlen( ABSPATH ) ) . '/uploads';

		$upload_path = get_site_option( 'ms_files_rewriting' ) ? false : $base_path . '/sites/' . $blog_id;
